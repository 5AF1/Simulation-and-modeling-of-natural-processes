...
# ffmpeg -r 15 -i bodies3D_%06d.png -q:v 0 bodies3D.avi
    #ax = plt.gcf().add_subplot(111, projection='3d') #, aspect='equal'
    ax = Axes3D(plt.gcf())
...
bodies = [Node(mass, px, py, pz) for (px,py,pz) in zip(posx, posy, posz) if (px-0.5)**2 + (py-0.5)**2 + (pz-0.5)**2 < ini_radius**2 ]
...
for i in range(max_iter):
    ...
    if i == 499 or i == 500 or i == 501:
        print(f"iteration no{i}: {bodies[0].pos()} {bodies[0].pos()[0]}")
        #ans .49855994
